"""Fail-closed TP-Link Deco client for the isolated channel laboratory.
...
LAB_CHANNEL = 11
LAB_BANDWIDTH = "HT20"
ROLLBACK_CHANNELS = frozenset(range(1, 14))
ROLLBACK_BANDWIDTHS = frozenset({"HT20", "HT40"})
...
class DecoLabClient:
    """Client with two reads, one fixed candidate write, and captured rollback."""
    ...
    async def apply_channel_11_ht20(self) -> None:
        """Apply the laboratory's single admitted candidate."""

        async with self._operation_lock:
            await self._authenticated_write(
                channel=LAB_CHANNEL,
                bandwidth=LAB_BANDWIDTH,
            )

    async def restore_captured_radio(self, channel: int, bandwidth: str) -> None:
        """Restore only a validated channel/width pair captured immediately before use."""

        if isinstance(channel, bool) or channel not in ROLLBACK_CHANNELS:
            raise ProbeProtocolError
        if bandwidth not in ROLLBACK_BANDWIDTHS:
            raise ProbeProtocolError
        async with self._operation_lock:
            await self._authenticated_write(channel=channel, bandwidth=bandwidth)
    ...
    async def _authenticated_write(self, channel: int, bandwidth: str) -> None:
        await self._login_if_needed()
        result = await self._encrypted_call(
            area="wireless",
            form="wlan",
            payload={
                "operation": "write",
